...
func main() {
    ...
    if err != nil { klog.Fatalf("Failed to listen on unix socket. error: %v", err) }

    cfg, err := rest.InClusterConfig()
    if err != nil { klog.Fatalf("Can not get cluster config. error: %v", err) }

    clientset, err := kubernetes.NewForConfig( cfg )
    if err != nil { klog.Fatalf("Can not initialize kubernetes client. error: %v", err) }

    defer listener.Close()
    providerSrv, err := server.NewServer( provider.NewSecretProviderFactory, clientset.CoreV1() )
    if err != nil { klog.Fatalf("Could not create server. error: %v", err) }
