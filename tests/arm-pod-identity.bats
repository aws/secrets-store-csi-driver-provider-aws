...
CLUSTER_NAME=integ-cluster-arm-pod-identity
POD_NAME=basic-test-mount-arm-pod-identity
...
log() {
	...
	echo -e "${YELLOW}[$TIMESTAMP] [arm-pod-identity] $msg${NC}" >&3
}
...
setup_file() {
	...
	KUBECONFIG_FILE_ARM_POD_IDENTITY=$(mktemp)
	...
	ROLE_ARN=$(aws --region "$REGION" --query Role.Arn --output text iam create-role --role-name arm-pod-identity-role --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
	...
	aws iam attach-role-policy \
		--role-name arm-pod-identity-role \
	...
	eksctl create podidentityassociation \
		...
		--namespace $NAMESPACE \
		--region $REGION \
		--service-account-name basic-test-mount-sa-arm-pod-identity \
		--role-arn $ROLE_ARN \
	...
	KUBECONFIG=$KUBECONFIG_FILE_ARM_POD_IDENTITY helm --namespace=$NAMESPACE install csi-secrets-store secrets-store-csi-driver/secrets-store-csi-driver --set enableSecretRotation=true --set rotationPollInterval=15s --set syncSecret.enabled=true
...
teardown_file() {
	...
	log "Deleting IAM role: arm-pod-identity-role"
	aws iam detach-role-policy --role-name arm-pod-identity-role --policy-arn arn:aws:iam::aws:policy/AmazonSSMReadOnlyAccess
	aws iam detach-role-policy --role-name arm-pod-identity-role --policy-arn arn:aws:iam::aws:policy/SecretsManagerReadWrite
	aws iam delete-role --role-name arm-pod-identity-role

	log "Cleaning up kubeconfig file"
	rm -f $KUBECONFIG_FILE_ARM_POD_IDENTITY
...
validate_jmes_mount() {
	...
	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/$USERNAME_ALIAS)
	[[ "${result//$'\r'}" == $USERNAME ]]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/$PASSWORD_ALIAS)
	[[ "${result//$'\r'}" == $PASSWORD ]]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/$SECRET_FILE_NAME)
	[[ "${result//$'\r'}" == $SECRET_FILE_CONTENT ]]

	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY get secret --namespace $NAMESPACE $K8_SECRET_NAME
	[ "$status" -eq 0 ]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace=$NAMESPACE get secret $K8_SECRET_NAME -o jsonpath="{.data.username}" | base64 -d)
	[[ "$result" == $USERNAME ]]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace=$NAMESPACE get secret $K8_SECRET_NAME -o jsonpath="{.data.password}" | base64 -d)
	[[ "$result" == $PASSWORD ]]
...
@test "Install aws provider" {
	...
	envsubst < $PROVIDER_YAML | kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY apply -f -
	cmd="kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE wait --for=condition=Ready --timeout=60s pod -l app=csi-secrets-store-provider-aws"
	wait_for_process $WAIT_TIME $SLEEP_TIME "$cmd"

	PROVIDER_POD=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE get pod -l app=csi-secrets-store-provider-aws -o jsonpath="{.items[0].metadata.name}")
	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE get pod/$PROVIDER_POD
	assert_success
...
@test "secretproviderclasses crd is established" {
	...
	cmd="kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY wait --namespace $NAMESPACE --for condition=established --timeout=60s crd/secretproviderclasses.secrets-store.csi.x-k8s.io"
	wait_for_process $WAIT_TIME $SLEEP_TIME "$cmd"

	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY get crd/secretproviderclasses.secrets-store.csi.x-k8s.io
	assert_success
...
@test "deploy aws secretproviderclass crd" {
	...
	envsubst < BasicTestMountSPC-ARM-PodIdentity.yaml | kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE apply -f -

	cmd="kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE get secretproviderclasses.secrets-store.csi.x-k8s.io/basic-test-mount-spc-arm-pod-identity -o yaml | grep aws"
	wait_for_process $WAIT_TIME $SLEEP_TIME "$cmd"
...
@test "CSI inline volume test with pod portability" {
	...
	kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE apply -f BasicTestMount-ARM-PodIdentity.yaml
	cmd="kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE wait --for=condition=Ready --timeout=60s pod/basic-test-mount-arm-pod-identity"
	wait_for_process $WAIT_TIME $SLEEP_TIME "$cmd"

	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE get pod/$POD_NAME
	assert_success
...
@test "CSI inline volume test with rotation - parameter store " {
	...
	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/ParameterStoreRotationTest)
...
@test "CSI inline volume test with rotation - secrets manager " {
	...
	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/SecretsManagerRotationTest)
	[[ "${result//$'\r'}" == "BeforeRotation" ]]
...
@test "CSI inline volume test with pod portability - read ssm parameters from pod" {
	...
	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/ParameterStoreTest1)
	[[ "${result//$'\r'}" == "ParameterStoreTest1Value" ]]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/ParameterStoreTest2)
...
@test "CSI inline volume test with pod portability - read secrets manager secrets from pod" {
	...
	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/SecretsManagerTest1)
	[[ "${result//$'\r'}" == "SecretsManagerTest1Value" ]]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE exec $POD_NAME -- cat /mnt/secrets-store/SecretsManagerTest2)
...
@test "Sync with Kubernetes Secret" {
	...
	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY get secret --namespace $NAMESPACE secret
	[ "$status" -eq 0 ]

	result=$(kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace=$NAMESPACE get secret secret -o jsonpath="{.data.username}" | base64 -d)
	[[ "$result" == "SecretUser" ]]
...
@test "Sync with Kubernetes Secret - Delete deployment. Secret should also be deleted" {
	...
	run kubectl --kubeconfig=$KUBECONFIG_FILE_ARM_POD_IDENTITY --namespace $NAMESPACE delete -f BasicTestMount-ARM-PodIdentity.yaml
	assert_success

	log "Waiting for secret to be deleted"
	run wait_for_process $WAIT_TIME $SLEEP_TIME "check_secret_deleted secret $NAMESPACE $KUBECONFIG_FILE_ARM_POD_IDENTITY"
	assert_success
