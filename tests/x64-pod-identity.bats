...
if [[ -z "${NODE_TYPE_X64_POD_IDENTITY}" ]]; then
	NODE_TYPE_X64_POD_IDENTITY=m5.large
fi
...
setup_file() {
	...
	log "Creating EKS cluster with node type $NODE_TYPE_X64_POD_IDENTITY"
	eksctl create cluster \
		--name $CLUSTER_NAME \
		--node-type $NODE_TYPE_X64_POD_IDENTITY \
		--nodes 3 \
		--region $REGION \
